...
switch ($action) {
    case 'getStaffName':
        ...
        if (strlen($staffid) != 6) {
            echo json_encode("StaffID format is not accepted, Contact Administrator");
        } else {
            $staffname = get_StaffByID($staffid);
            if ($staffname != 'empty') {
                echo json_encode($staffname['name']);
            } else {
                echo json_encode("Cannot find Staff Name for StaffID = $staffid");
            }
        }
        break;
    case 'getParseJobCode':
        $jobcode = $received_data->jobcode;
        #echo "Jobcode = $jobcode\n";
        try {
            $parsedJobCode = parseJobcode($jobcode);
            if ($parsedJobCode == 'fail') {
                throw new Exception("<span style='color:red'>Cannot parse Jobcode -> <font style='color:yellow'>$jobcode</font></span>");
            } else {
                echo json_encode("<font style='color:yellow'>$parsedJobCode</font>");
            }
        } catch (Exception $ex) {
                ...
                case 'start': //start process, insert to database
                    ...
                    if ($numrowspot != 0) {//if already started / ended
                        ...
                        if ($insResult == 'insert ok!') { //if insert succesful
                            ...
                            $poid = $results['poid'];
                            $successMsg = "Job No <strong><font color=\"#FFFF00\">$parseJobCode</font></strong> has been started.<br>";
