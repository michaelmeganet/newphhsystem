...
if ($_GET['jte'] && $_GET['sfid'] && $_GET['jno'] && $_GET['bid']) {
    ...
    <table width="50%" cellspacing="0" cellpadding="2" border="0">
	  ...
	  If you believe this is an error, please contact your administrator regarding this thing.</td>
            ...
            $sqlprocount = "SELECT COUNT(*) FROM $protab WHERE quono LIKE '$quono%' AND runningno = $runningno AND noposition = $itemno  AND (jlfor = '$branch' OR (operation = 1 OR operation = 3)) AND $addi";
            $objSqlprocount = new SQL($sqlprocount);
            $numrowspro = $objSqlprocount->getRowCount();
            ...
            if ($numrowspro != 0) {
                ...
                $sqlpotcount = "SELECT COUNT(*) FROM $pottab WHERE sid = {$rowpro['sid']} AND jobtype = '$jobtype' ORDER BY poid";
                #$resultpot = $rundb->Query($sqlpot);
                #$numrowspot = $rundb->NumRows($resultpot);
                $objSqlpotcount = new SQL($sqlpotcount);
                $numrowspot = $objSqlpotcount->getRowCount();
                if ($numrowspot == 0) {
                    echo "<tr>
	      <td colspan=\"5\">$jobtypename Job No <strong><font color=\"#FFFF00\">$branch $companycode $quodat $runningno $itemno $additional</font></strong> has not been started.</td>
                    ...
                    $resultpot = $objSqlpot->getResultRowArray();
                    foreach ($resultpot as $rowpot){
